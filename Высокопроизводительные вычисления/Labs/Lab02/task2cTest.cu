...
int main()
{
  ...
  int full_size = N * N;
  int part_size = div_up(full_size, CUDA_STREAMS_NUM);
  //int mem_full_size = N * N * sizeof(float);
  //int mem_part_size = mem_full_size / CUDA_STREAMS_NUM;
  int mem_full_size = full_size * sizeof(float);
  int mem_part_size = part_size * sizeof(float);
  float* adev = createMtrxOnDevice(N);
  float* bdev = createMtrxOnDevice(N);
  for (int i = 0; i < CUDA_STREAMS_NUM-1; i++)
  {
  ...
  int i = CUDA_STREAMS_NUM - 1;
  int tmp_part_mem_size = mem_full_size % mem_part_size;
  if (tmp_part_mem_size == 0)
    tmp_part_mem_size = mem_part_size;
  cudaMemcpyAsync(adev + i * part_size, a + i * part_size, tmp_part_mem_size, cudaMemcpyHostToDevice, stream[i]);
  cudaMemcpyAsync(bdev + i * part_size, b + i * part_size, tmp_part_mem_size, cudaMemcpyHostToDevice, stream[i]);
  ...
  double blocks_n_float = sqrt(div_up(N * N, (threads.x * threads.y * CUDA_STREAMS_NUM)));
  int blocks_n = std::ceil(blocks_n_float);
  dim3 blocks(blocks_n, blocks_n);
  ...
  /*for (int i = 0; i < CUDA_STREAMS_NUM; i++)
  {
    auto cuda_err = cudaMemcpyAsync(d + i * part_size, cdev + i * part_size, mem_part_size, cudaMemcpyDeviceToHost, stream[i]);
    if (cuda_err != cudaSuccess)
      printf("cudaMemcpyAsync() error %d: %s\n", cuda_err, cudaGetErrorString(cuda_err));
  }*/
  for (int i = 0; i < CUDA_STREAMS_NUM - 1; i++)
    cudaMemcpyAsync(d + i * part_size, cdev + i * part_size, mem_part_size, cudaMemcpyDeviceToHost, stream[i]);
  i = CUDA_STREAMS_NUM - 1;
  tmp_part_mem_size = mem_full_size % mem_part_size;
  if (tmp_part_mem_size == 0)
    tmp_part_mem_size = mem_part_size;
  cudaMemcpyAsync(d + i * part_size, cdev + i * part_size, tmp_part_mem_size, cudaMemcpyDeviceToHost, stream[i]);
  ...
  while (n > 0)
  {
    ...
    for (int it = 0; it < ITERS; it++)
    {
      ...
      int id = CUDA_STREAMS_NUM - 1;
      int tmp_part_mem_size = mem_full_size % mem_part_size;
      if (tmp_part_mem_size == 0)
        tmp_part_mem_size = mem_part_size;
      cudaMemcpyAsync(adev2 + id * part_size, a2 + id * part_size, tmp_part_mem_size, cudaMemcpyHostToDevice, stream[id]);
      cudaMemcpyAsync(bdev2 + id * part_size, b2 + id * part_size, tmp_part_mem_size, cudaMemcpyHostToDevice, stream[id]);*/
      ...
      /*for (int i = 0; i < CUDA_STREAMS_NUM - 1; i++)
        cudaMemcpyAsync(d2 + i * part_size, cdev2 + i * part_size, mem_part_size, cudaMemcpyDeviceToHost, stream[i]);
      id = CUDA_STREAMS_NUM - 1;
      tmp_part_mem_size = mem_full_size % mem_part_size;
      if (tmp_part_mem_size == 0)
        tmp_part_mem_size = mem_part_size;
      cudaMemcpyAsync(d2 + id * part_size, cdev2 + id * part_size, tmp_part_mem_size, cudaMemcpyDeviceToHost, stream[id]);*/
